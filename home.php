...
function filterDomainInfo($conn) {
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $domainFilter[] = $row['domain_name'];
        }
    }
    return $domainFilter;
}

$domainNames= filterDomainInfo($conn);
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $backgroundImages[$row['domain_id']] = $row['domain_image'];
            ...
            <div class="filter-box" id="filterBox">
                ...
                <div class="filter-content">
                    ...
                    <?php foreach ($domainNames as $domainName): ?>
                        <label><input type="checkbox" value="<?php echo $domainName; ?>"style="margin-right:10px; margin-bottom:5px;"><?php echo $domainName; ?></label>
                    <?php endforeach; ?>
                    </div>
